# FIXME all this Javascript mojo does not work for us. What do we do?
...
def format_status(status_or_checkout, input_io = $stdin)
  input = input_io.read

  if input.length < 1
    make_head( "Hg Status", work_path,
             [ ENV['TM_BUNDLE_SUPPORT'] + "/Stylesheets/hg_style.css",
               ENV['TM_BUNDLE_SUPPORT'] + "/Stylesheets/hg_status_style.css"] )
...
ENDJS
  ...
  make_head( "Hg Status", work_path,
             [ ENV['TM_BUNDLE_SUPPORT'] + "/Stylesheets/hg_style.css",
               ENV['TM_BUNDLE_SUPPORT'] + "/Stylesheets/hg_status_style.css"], js_functions )
